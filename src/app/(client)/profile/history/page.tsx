...
export default function HistoryPage() {
  ...
  const statusConfig = {
    served:    { label: t.statusServed,    icon: CheckCircle, color: "#07984a" },
    cancelled: { label: t.statusCancelled, icon: XCircle,     color: "#ef2b24" },
    waiting:   { label: t.statusWaiting,   icon: Clock,       color: "#ff9300" },
  };
  ...
  return (
    <div className="bg-surface-2 min-h-svh">
      <div className="sticky top-0 z-30 bg-white/95 backdrop-blur-sm px-4 pt-safe-top pb-3 border-b border-line flex items-center gap-3">
        <button onClick={() => router.back()} className="w-10 h-10 rounded-full bg-surface-2 flex items-center justify-center" aria-label={t.back}>
          <CaretLeft weight="bold" size={20} />
        </button>
        <h1 className="text-xl font-black text-ink">{t.history}</h1>
        <span className="ml-auto text-xs font-bold text-ink-3 bg-surface-2 border border-line rounded-full px-3 py-1">
          {HISTORY.length} {t.visits}
        </span>
      </div>
